...
def run_full_drc(layout_name, output_file):
    ...
    if os.access(magpath, os.W_OK):
        ...
        return
    print(f'Evaluating full DRC results for layout {layout_name}')
    magic_script = (
        scriptpath + f"/run_magic_drc_{os.path.basename(layout_name)}.tcl"
    )
    ...
    with open(magic_script, 'w') as ofile:
        ...
        if is_gds:
            ...
            print(f'load {layout_name}', file=ofile)
        else:
        ...
        print(f'set ofile [open {output_file} w]', file=ofile)
    ...
    print(f'Running: magic -dnull -noconsole -rcfile {rcfile} {magic_script}')
